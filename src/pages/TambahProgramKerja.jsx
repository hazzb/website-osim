...
// --- VERSI 2.5 (Menggunakan Embed HTML) ---

import React, { useState, useEffect, useMemo } from 'react';
import { supabase } from '../supabaseClient';
import { useNavigate, useLocation } from 'react-router-dom';

// ... (useQuery - tidak berubah) ...
function useQuery() {
...
function TambahProgramKerja() {
  ...
  const [status, setStatus] = useState(urlStatus || 'Rencana');
  ...
  // --- (Efek 1, 2, 3 - tidak berubah dari v2.5 sebelumnya) ---
  useEffect(() => {
    async function fetchPeriode() {
      setLoadingPeriode(true);
      try {
        const { data, error } = await supabase.from('periode_jabatan').select('id, nama_kabinet, tahun_mulai, tahun_selesai, is_active').order('tahun_mulai', { ascending: false });
        if (error) throw error;
        setPeriodeList(data || []);
        if (!urlPeriodeId) {
          const activePeriode = data.find(p => p.is_active);
          if (activePeriode) setSelectedPeriodeId(activePeriode.id);
          else if (data.length > 0) setSelectedPeriodeId(data[0].id);
        }
      } catch (error) { alert("Gagal memuat daftar periode: " + error.message); } 
  ...
  useEffect(() => {
    ...
    async function fetchDivisi() {
      ...
      try {
        const { data, error } = await supabase.from('divisi').select('id, nama_divisi').eq('periode_id', selectedPeriodeId).order('urutan', { ascending: true }).order('nama_divisi', { ascending: true });
        if (error) throw error;
  ...
  useEffect(() => {
    setAnggotaList([]); setSelectedPjId('');
    ...
    fetchAnggota();
  }, [selectedDivisiId, selectedPeriodeId]);
    ...
    try {
      const { error } = await supabase
        ...
        .insert([{ 
            ...
            embed_html: embedHtml, // <-- KIRIM DATA BARU
    ...
    <div>
      ...
      <form style={formStyle} onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
          <div style={{ ...inputGroupStyle, flex: 1, minWidth: '200px' }}><label style={labelStyle} htmlFor="periode">1. Periode:</label><select id="periode" style={selectStyle} value={selectedPeriodeId} onChange={(e) => setSelectedPeriodeId(e.target.value)} disabled={loadingPeriode || !!urlPeriodeId} required><option value="" disabled>-- Pilih Periode --</option>{periodeList.map(p => <option key={p.id} value={p.id}>{p.tahun_mulai}/{p.tahun_selesai} ({p.nama_kabinet})</option>)}</select>{urlPeriodeId && <small>Periode diisi otomatis.</small>}</div>
          <div style={{ ...inputGroupStyle, flex: 1, minWidth: '200px' }}><label style={labelStyle} htmlFor="divisi">2. Divisi:</label><select id="divisi" style={selectStyle} value={selectedDivisiId} onChange={(e) => setSelectedDivisiId(e.target.value)} disabled={loadingDivisi || divisiList.length === 0} required><option value="" disabled>-- Pilih Divisi --</option>{divisiList.map(d => <option key={d.id} value={d.id}>{d.nama_divisi}</option>)}</select></div>
          <div style={{ ...inputGroupStyle, flex: 1, minWidth: '200px' }}><label style={labelStyle} htmlFor="pj">3. Penanggung Jawab (PJ):</label><select id="pj" style={selectStyle} value={selectedPjId} onChange={(e) => setSelectedPjId(e.target.value)} disabled={loadingAnggota || anggotaList.length === 0} required><option value="" disabled>-- Pilih PJ --</option>{anggotaList.map(a => <option key={a.id} value={a.id}>{a.nama}</option>)}</select></div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          ...
          <div style={{ ...inputGroupStyle, flex: 1 }}><label style={labelStyle} htmlFor="status">Status:</label><select id="status" style={selectStyle} value={status} onChange={(e) => setStatus(e.target.value)} disabled={!!urlStatus} required><option value="Rencana">Rencana</option><option value="Akan Datang">Akan Datang</option><option value="Selesai">Selesai</option></select>{urlStatus && <small>Status diisi otomatis.</small>}</div>
        </div>
