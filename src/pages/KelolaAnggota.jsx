  ...
  const handleBulkUpload = async (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const payload = [];
        const periodeName = periodeList.find(
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (!divId)
            throw new Error(`Baris ${rowNum}: Kolom 'ID Divisi' kosong.`);
          if (!validDiv)
            throw new Error(
              `Baris ${rowNum}: ID Divisi '${divId}' TIDAK VALID untuk Periode '${periodeName}'.`,
            );

          const jabId = row["ID Jabatan"];
          const validJab = jabatanList.find(
            (j) => String(j.id) === String(jabId),
          );

          if (jabId && !validJab) {
            throw new Error(
              `Baris ${rowNum}: ID Jabatan '${jabId}' tidak ditemukan.`,
            );
          }

          const jkRaw = row["Jenis Kelamin (L/P)"] || "L";
          const jk = jkRaw.toString().toUpperCase().includes("P")
            ? "Akhwat"
            : "Ikhwan";
            ...
            {selectedPeriodeId ? (
              <>
                ...
                >
                  ...
                  .
                  <br />
                  Gunakan <strong>ID Divisi</strong> &{" "}
                  <strong>ID Jabatan</strong> yang valid (Lihat tabel di bawah).
                </p>
              ...
              >
                ...
                >
                  ...
                  Untuk melakukan Import Excel, Anda harus memilih salah satu{" "}
                  <strong>Periode Kabinet</strong> di filter atas agar data
                  masuk ke tempat yang benar.
                </p>
              </div>
            )}
